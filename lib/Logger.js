...
class Logger {
     ...
     *
     * @param {string} name           - name of the Logger, can be found later
     *                                  in the log entries
     *
     * @param {object} config         - A configuration object for werelogs.
     *                                  Note that this argument is already
     *                                  bound into the constructor by the
     *                                  exported API, in order to provide an
     *                                  easy-to-use API, that handles
     *                                  configuration from a single place.
     ...
     *                                  'warn', 'error' and 'fatal' in order of
     *                                  importance.)
     * @param {object[]} config.streams   - The array of streams into which to
     *                                      log. Their configuration is
     *                                      directly related to the expected
     *                                      bunyan streams array, for
     *                                      compatibility purposes (except that
     *                                      the 'level' field is not accounted
     *                                      for)
     *
     *
    ...
    setLevel(levelName) {
        this.config.update({ level: levelName });
    }

    setDumpThreshold(levelName) {
        this.config.update({ dump: levelName });
    }
    /**
