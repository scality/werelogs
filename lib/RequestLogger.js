...
class EndLogger {
    ...
    augmentedLog(level, msg, data) {
        ...
        // call.
        if (data) {
            Object.keys(data).forEach( f => this.fields[f] = data[f]);
        }
    ...
    addDefaultFields(fields) {
        ...
        Object.keys(fields).forEach(f => this.fields[f] = fields[f]);
        return oldFields;
    }
...
class RequestLogger {
    ...
    log(level, msg, logFields, isEnd) {
        ...
        const fields = {};
        if (logFields) {
            Object.keys(logFields).forEach( f => fields[f] = logFields[f]);
        }
    ...
    doLogIO(logEntry) {
        ...
        Object.keys(logEntry.fields)
            .forEach(f => fields[f] = logEntry.fields[f]);
        Object.keys(this.fields).forEach(f => fields[f] = this.fields[f]);
