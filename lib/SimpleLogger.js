...
// NOTE: Added this require based on the project structure
...
class SimpleLogger {
    ...
    log(level, fields, message) {
        ...
        // OPTIMIZATION: Use lazy evaluation for the stringified log.
        // It will only be created if at least one stream needs it.
        let safeString = null;

        this.streams.forEach(s => {
            // Check if this specific stream should log this message level.
            if (LogLevel.shouldLog(level, s.level)) {
                // If this is the first stream to match, create the JSON string.
                if (safeString === null) {
                    // Assemble the full log object just-in-time.
                    ...
                    safeString = safeStringify(finalLogFields);
                }
